...
高级 API，将 `.pptx` 文件解析为 `PresentationOptions`，可直接传给 `generatePresentation()`。`parsePresentation` 和 `parsePptx` 均接受任意 `DataType` 输入；`parsePresentation` 为 async，额外接受 `Blob`/`ReadableStream`，同步对应版本为 `parsePresentationSync`。
