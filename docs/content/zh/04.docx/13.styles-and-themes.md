...
`default.language` 属性设置文档默认值的 `w:lang`——Word 从编辑语言派生它，影响生成文档的拼写检查与字体回退。默认为 `{ "value": "en-US" }`（en-US Word 安装）；zh-CN 安装则写入 `{ "value": "en-US", "eastAsia": "zh-CN", "bidirectional": "ar-SA" }`。
