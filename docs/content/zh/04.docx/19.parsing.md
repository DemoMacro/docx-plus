...
两个函数均接受任意 `DataType` 输入：`Uint8Array`、`ArrayBuffer`、`DataView`、`number[]`、base64 字符串等。`Blob`（含 `File`）与 `ReadableStream` 输入由 async 入口接受；每个高层 parse 都有同步的 `parse*Sync` 对应版本，接收已归一化的字节。
