...
Both functions accept any `DataType` input: `Uint8Array`, `ArrayBuffer`, `DataView`, `number[]`, base64 string, and more. `Blob` (including `File`) and `ReadableStream` inputs are accepted by the async entries; every high-level parse also has a synchronous `parse*Sync` counterpart that takes already-normalized bytes.
...
const opts = await parseWorkbook(readFileSync("input.xlsx"));
