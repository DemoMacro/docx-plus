...
The high-level API that parses a `.pptx` file into `PresentationOptions` that can be passed to `generatePresentation()`. Both `parsePresentation` and `parsePptx` accept any `DataType` input; `parsePresentation` is async and additionally takes `Blob`/`ReadableStream`, with `parsePresentationSync` as the synchronous counterpart.

```ts
import { parsePresentation, generatePresentation } from "@office-open/pptx";
import { readFileSync, writeFileSync } from "node:fs";

const opts = await parsePresentation(readFileSync("input.pptx"));
...
const parsed = await parsePresentation(readFileSync("template.pptx"));
...
const opts = await parsePresentation(arrayBuffer);
