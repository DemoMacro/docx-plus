...
The `default.language` property sets the document defaults' `w:lang` — Word derives it from the editing language, so it drives spell-check and font fallback for generated documents. It defaults to `{ "value": "en-US" }` (an en-US Word install); a zh-CN install writes `{ "value": "en-US", "eastAsia": "zh-CN", "bidirectional": "ar-SA" }`.
